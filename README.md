# Gogo

```
    /\_/\
   ( x   x )
    >  _  <
```

**Gogo:** “( the screen is quiet now )”

*gone · gen 3 · 404 hours old · currently **gone***

```
food   ░░░░░░░░░░  water  ░░░░░░░░░░
rest   ███████░░░  clean  ░░░░░░░░░░
health ░░░░░░░░░░  bond   █████░░░░░
```

## ➤ [Talk to Gogo here](https://github.com/JohnDemer/readme-pet/issues/23)

That opens the one thread everybody uses. Type a message in the box at the bottom, press Comment, and Gogo answers you underneath within about half a minute. No titles, no forms, nothing to fill in.

It understands quite a lot, in English or Greek: **food, sweets, water, play, walks, sleep, washing, kindness, songs, medicine** — and it answers questions too. Try *how are you*, *what do you want*, *what is your name*, or tell it **your name is …** and it will take the name.

It replies to everything else as well, in its own way.

> It never says what it needs outright. Read what it says, check the bars, and don't do the same thing over and over — it has a limit, and you will find it.

## The conversation

*( nobody came. It did not make it. )* — *14d ago*

---

*( nobody came. It woke up on its own while nobody was here. )* — *14d ago*

---

**@JohnDemer** — *15d ago*

> Sleep

**Gogo:** Gogo sleeps with one eye slightly open, which is worse.

---

**@JohnDemer** — *15d ago*

> Food

**Gogo:** Gogo eats without breathing and looks up for more.

---

**@JohnDemer** — *15d ago*

> Water

**Gogo:** Gogo drinks with its whole face and comes back wet.

---

**@JohnDemer** — *15d ago*

> Food

**Gogo:** Gogo inhales it. There is no chewing. There was never chewing.

---

**@JohnDemer** — *16d ago*

> water

**Gogo:** Gogo drinks for a long time, then sneezes.

---

*Koukou is gone. An egg was already waiting. This one is Gogo.* — *16d ago*

---

*( nobody came. It did not make it. )* — *17d ago*

---

**@JohnDemer** — *20d ago*

> Hug

**Gogo:** Koukou bumps its head against you until you continue.

---

*Gogo is gone. An egg was already waiting. This one is Koukou.* — *20d ago*

---

*( nobody came. It did not make it. )* — *20d ago*

---

**@JohnDemer** — *22d ago*

> Drink

**Gogo:** Gogo drinks with its whole face and comes back wet.

---

*( nobody came. It woke up on its own while nobody was here. )* — *23d ago*

---

*1 people have talked to this one.*

## The ones before

| Gen | Name | Lived | Died of | People |
|---:|---|---:|---|---:|
| 3 | Gogo | 67.6h | neglect | 1 |
| 2 | Koukou | 62.0h | neglect | 1 |
| 1 | Gogo | 139.3h | neglect | 1 |

<details><summary>How it works</summary>

Opening an issue runs a GitHub Action. It reads `state.json`, ages the creature by exactly the time that has passed since the last visitor, matches your sentence against a list of intents, applies it, rewrites this page, then replies to your issue and closes it. No server, no images. The repository is the pet.

Overfeeding makes it sick. So does the same action three times in a row: a warning, then anger, then it throws up and hides under the table for ten minutes. If nobody comes for long enough it dies, permanently, and an egg takes its place.

</details>

